#!/usr/bin/env python3
"""
Export LinkedIn carousel (1080x1080), one-pager (A4), and whitepaper (A4 multi-page) to PDF.
...
WHITEPAPER_HTML = SCRIPT_DIR / "html" / "whitepaper.html"
WHITEPAPER_PDF  = SCRIPT_DIR / "html" / "whitepaper.pdf"
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # 3. Whitepaper — A4 multi-page
        page = await browser.new_page(viewport={"width": 794, "height": 1123})
        await page.goto(f"file://{WHITEPAPER_HTML.resolve()}", wait_until="networkidle")
        await page.wait_for_timeout(1200)
        await page.pdf(
            path=str(WHITEPAPER_PDF),
            format="A4",
            print_background=True,
            margin={"top": "0", "bottom": "0", "left": "0", "right": "0"},
        )
        print(f"OK Whitepaper PDF: {WHITEPAPER_PDF}")
